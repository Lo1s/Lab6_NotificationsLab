...
public class DownloaderTaskFragment extends Fragment {
	...
	private ArrayList<Integer> sRawTextFeedIds = new ArrayList<Integer>();
	private Integer[] textFeedIds;
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		sRawTextFeedIds = getArguments().getIntegerArrayList(MainActivity.TAG_FRIEND_RES_IDS);
		textFeedIds = new Integer[sRawTextFeedIds.size()];
		for (int i = 0; i < sRawTextFeedIds.size(); i++) {
			textFeedIds[i] = sRawTextFeedIds.get(i);
		}
		
		
		// TODO: Start the DownloaderTask
		task.execute(textFeedIds);
						...
						public void onReceive(Context context, Intent intent) {
							...
							if (!(MainActivity.IS_ALIVE == getResultCode())) {
								...
								if (success)
									...
									mContentView.setTextViewText(R.id.text,
											"Download has failed. Please retry Later.");
